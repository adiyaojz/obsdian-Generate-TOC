const { Plugin, PluginSettingTab, Setting, ColorComponent } = require('obsidian');
...
class UpdateAdTocPlugin extends Plugin {
    ...
    updateAdTocBlocks() {
        ...
        if (this.settings.enableCodeBlock) {
            ...
            if (this.settings.enableTitleContent && this.settings.titleContent !== 'none') {
                newAdTocContent += `title: ${this.settings.titleContent}\n`;
            }
            if (this.settings.enableColorContent && this.settings.colorContent !== 'none') {
                newAdTocContent += `color: ${this.settings.colorContent}\n`;
            }
            if (this.settings.enableCollapseContent && this.settings.collapseContent !== 'none') {
...
module.exports = UpdateAdTocPlugin;

function heading(arr, l = 3) {
    const generateSpaces = (level) => ' '.repeat((level - 1) * 4);
    return arr.filter(item => item.level <= l)
               .map(item => {
...
class AdTocSettingTab extends PluginSettingTab {
    ...
    display() {
        ...
        codeBlockSettings.style.display = this.plugin.settings.enableCodeBlock ? 'block' : 'none';
        ...
        titleSettings.style.display = this.plugin.settings.enableTitleContent ? 'block' : 'none';

        new Setting(titleSettings)
            .setName(texts[lang].titleContent)
            .setDesc(texts[lang].customTitleContent)
            .addText(text => text
                .setValue(this.plugin.settings.titleContent !== 'none' ? this.plugin.settings.titleContent : '')
                .onChange(async (value) => {
                    if (value.trim() === '') {
        ...
        colorSettings.style.display = this.plugin.settings.enableColorContent ? 'block' : 'none';

        new Setting(colorSettings)
            .setName(texts[lang].colorContent)
            .setDesc(texts[lang].customColor)
            .addColorPicker(color => color
                .setValue(this.plugin.settings.colorContent !== 'none' ? this.plugin.settings.colorContent : '#000000')
                .onChange(async (value) => {
        ...
        collapseSettings.style.display = this.plugin.settings.enableCollapseContent ? 'block' : 'none';
